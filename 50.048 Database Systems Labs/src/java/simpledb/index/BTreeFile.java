...
public class BTreeFile implements DbFile {
	...
	throws DbException, TransactionAbortedException {
		...
		// If the page is a leaf page, return it
		if (pid.pgcateg() == BTreePageId.LEAF) {
			return (BTreeLeafPage) page;
		} 
		// If the page is an internal page, find the child pointer that should be followed
		...
		// If the page is not a leaf or internal page, throw an exception
		else {
	...
	public BTreeInternalPage splitInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		// Get the entries in reverse order
		...
		// Push the middle key up to the parent page
		BTreeEntry e = entries.next();
		Field index = e.getKey();
		page.deleteKeyAndRightChild(e);

		// Insert the new entry into the parent page
	...
	public void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		if (t == null) {
			throw new DbException("Failed to move tuples.");
		}

		// Update parent entry with the key from the last moved tuple
	...
	public void stealFromLeftInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
			...
			BTreeEntry parentEntry) throws DbException, TransactionAbortedException { // Lab 4
		// some code goes here
        // Move some of the entries from the left sibling to the page so
		// that the entries are evenly distributed. Be sure to update
		// the corresponding parent entry. Be sure to update the parent
		// pointers of all children in the entries that were moved.
		if (page == null || leftSibling == null || parent == null || parentEntry == null) {
			throw new IllegalArgumentException("Page, leftSibling, parent, and parentEntry cannot be null");
		}
	
		Iterator<BTreeEntry> siblingEntryIterator = leftSibling.reverseIterator();
		int entriesToMove = (leftSibling.getNumEntries() - page.getNumEntries()) / 2;
	
		if (entriesToMove <= 0) {
			throw new DbException("No entries to move from left sibling to page.");
		}
	
		// Move parent's entry to page
		BTreeEntry movingEntry = siblingEntryIterator.next();
		BTreeEntry newParentEntry = new BTreeEntry(parentEntry.getKey(), movingEntry.getRightChild(), page.iterator().next().getLeftChild());
		page.insertEntry(newParentEntry);
	
		// Take out the sibling entry and insert it into the page
		for (int i = 0; i < entriesToMove - 1; ++i) {
			if (!siblingEntryIterator.hasNext()) {
				throw new DbException("Sibling has no more entries.");
			}
			leftSibling.deleteKeyAndRightChild(movingEntry);
			page.insertEntry(movingEntry);
			movingEntry = siblingEntryIterator.next();
		}
		leftSibling.deleteKeyAndRightChild(movingEntry);
		parentEntry.setKey(movingEntry.getKey());
	...
	public void stealFromRightInternalPage(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		// Check if page, rightSibling, parent, and parentEntry are null
		if (page == null || rightSibling == null || parent == null || parentEntry == null) {
			throw new IllegalArgumentException("Page, leftSibling, parent, and parentEntry cannot be null");
		}
	
		Iterator<BTreeEntry> siblingEntryIterator = rightSibling.iterator();
		int entriesToMove = (rightSibling.getNumEntries() - page.getNumEntries()) / 2;
		...
		// Move parent's entry to page
		BTreeEntry movingEntry = siblingEntryIterator.next();
		BTreeEntry newParentEntry = new BTreeEntry(parentEntry.getKey(), page.reverseIterator().next().getRightChild(), movingEntry.getLeftChild());
		page.insertEntry(newParentEntry);
		...
		for (int i = 0; i < entriesToMove - 1; ++i) {
			...
			rightSibling.deleteKeyAndLeftChild(movingEntry);
			page.insertEntry(movingEntry);
			movingEntry = siblingEntryIterator.next();
		}
		rightSibling.deleteKeyAndLeftChild(movingEntry);
	...
	public void mergeLeafPages(TransactionId tid, Map<PageId, Page> dirtypages,
			BTreeLeafPage leftPage, BTreeLeafPage rightPage, BTreeInternalPage parent, BTreeEntry parentEntry) 
					throws DbException, IOException, TransactionAbortedException { // Lab 4
		...
		// Check if leftPage, rightPage, parent, and parentEntry are null
		if (leftPage == null || rightPage == null || parent == null || parentEntry == null) {
			throw new IllegalArgumentException("leftPage, rightPage, parent, and parentEntry cannot be null");
		}

		// Get the parent entry key
		Field parentEntryKey = parentEntry.getKey();

		// Get the tuples from the right page
		...
		if (tuples.isEmpty()) {
			throw new DbException("No tuples found in rightPage");
		}

		// Move the tuples from the right page to the left page
	...
	public void mergeInternalPages(TransactionId tid, Map<PageId, Page> dirtypages,
		...
		// Get the entries from the right page
		Iterator<BTreeEntry> it = rightPage.iterator();
		if (!it.hasNext()) {
			throw new DbException("No entries found in rightPage");
		}

		// First, insert the index node in parent into the left page
		...
		// Move the entries from the right page to the left page
		while (it.hasNext()) {
